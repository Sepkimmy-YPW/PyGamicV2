...
class OrigamiSimulator:
    ...
    def startOnlyTSA(self, units, max_size, total_bias, unit_edge_max):
        ...
        self.bm = ti.Matrix.field(2, 2, dtype=data_type, shape=self.div_indices_num)
        
        self.W = ti.field(dtype=data_type, shape=self.div_indices_num)
    ...
    @ti.func
    def calculateStringDistanceParallel(self, cur_angle: ti.f64):
        accumulate_index = 0
    ...
    @ti.kernel
    def calculateStringDistance(self, cur_angle: ti.f64):
    ...
    def implicit_xv(self, theta: ti.f64, sim_mode: bool, gravitational_acc: tm.vec3, enable_ground: bool):
        for i in range(self.kp_num):
    ...
    def getEnergy(self, sim_mode, theta):
        ...
        for i in range(self.div_indices_num):
            ...
            #stvk model
            green_tensor = 0.5 * (f.transpose() @ f - ti.Matrix.identity(data_type, 2))
            # energy tensor
            energy_tensor = 0.0
    ...
    def F(self, theta: data_type, sim_mode: bool, gravitational_acc: tm.vec3, enable_ground: bool, tsa_turning_angle: data_type):
        ...
        for i in range(self.div_indices_num):
            ...
            #stress energy
            psi = self.mu * energy_tensor + self.landa / 2.0 * ti.Matrix.trace(green_tensor) ** 2
            piola = f @ (2.0 * self.mu * green_tensor + self.landa * ti.Matrix.trace(green_tensor) * ti.Matrix.identity(data_type, 2))  
