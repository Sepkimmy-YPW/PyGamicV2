...
class OrigamiSimulator:
    ...
    def commonStart_1(self, unit_edge_max):
        ...
        for i in range(self.completed_constraint_number):
            self.tsa_string_number += self.completed_constraint_number - 1 # will multi 2
    ...
    def commonStart_2(self, unit_edge_max):
        ...
        self.grad_x = ti.field(data_type, shape=3 * self.kp_num, needs_dual=True)
        self.grad_f = ti.field(data_type, shape=3 * self.kp_num, needs_dual=True)
    ...
    def initialize(     self, 
        ...
        for i in ti.ndrange(self.total_root_number - self.actuation_number):
            ...
            self.constraint_start_point_duplicate[i + self.total_root_number - self.actuation_number] = [
                tm.cos(start_id / self.panel_resolution * 2 * tm.pi) * self.panel_size, 
                tm.sin(start_id / self.panel_resolution * 2 * tm.pi) * self.panel_size, 
                self.origami_z_bias
            ]
        ...
        for i in ti.ndrange(self.total_root_number - self.actuation_number):
            self.constraint_start_point[i + accumulate_index] = [
                self.constraint_start_point_duplicate[i + self.total_root_number - self.actuation_number][X], 
                self.constraint_start_point_duplicate[i + self.total_root_number - self.actuation_number][Y], 
                self.origami_z_bias
            ]
            self.constraint_point_number[i + self.total_root_number - self.actuation_number] = 1
    ...
    @ti.func
    def getEnergy(self, sim_mode: bool, theta: data_type) -> data_type:
        total_energy = 0.0
        for i in range(self.div_indices_num):
        ...
        facet_k = 0.0
        if sim_mode == self.FOLD_SIM:
            facet_k = self.ori_sim.bending_k * self.lames_bonus[0]
        else:
            facet_k = 15. * self.ori_sim.bending_k * self.lames_bonus[0]
        ...
        for i in range(self.kp_num):
            total_energy += 0.5 * self.masses[i] * self.v[i].norm() ** 2
        return total_energy
    ...
    def F(self, theta: data_type, sim_mode: bool, gravitational_acc: tm.vec3, enable_ground: bool, tsa_turning_angle: data_type):
        ...
        for i in range(self.div_indices_num):
            ...
            x0 = tm.vec3([self.grad_x[indice_x0], self.grad_x[indice_x0 + 1], self.grad_x[indice_x0 + 2]])
            x1 = tm.vec3([self.grad_x[indice_x1], self.grad_x[indice_x1 + 1], self.grad_x[indice_x1 + 2]])
            x2 = tm.vec3([self.grad_x[indice_x2], self.grad_x[indice_x2 + 1], self.grad_x[indice_x2 + 2]])
            ...
            self.force[indice_x0] += f0
            self.force[indice_x1] += f1
            self.force[indice_x2] += f2
            ...
            for j in range(3):
                self.grad_f[indice_x0 + j] += f0[j]
                self.grad_f[indice_x1 + j] += f1[j]
                self.grad_f[indice_x2 + j] += f2[j]
        ...
        if sim_mode == self.FOLD_SIM:
            for i in range(self.crease_pairs_num):
                ...
                # cs = tm.vec3([self.grad_x[3 * crease_start_index], self.grad_x[3 * crease_start_index + 1], self.grad_x[3 * crease_start_index + 2]])
                # ce = tm.vec3([self.grad_x[3 * crease_end_index], self.grad_x[3 * crease_end_index + 1], self.grad_x[3 * crease_end_index + 2]])
                # p1 = tm.vec3([self.grad_x[3 * related_p1], self.grad_x[3 * related_p1 + 1], self.grad_x[3 * related_p1 + 2]])
                # p2 = tm.vec3([self.grad_x[3 * related_p2], self.grad_x[3 * related_p2 + 1], self.grad_x[3 * related_p2 + 2]])

                # 计算弯曲力
                csf, cef, rpf1, rpf2, energy, current = self.getBendingForce(
                    self.get_position_with_index(crease_start_index), 
                    self.get_position_with_index(crease_end_index), 
                    self.get_position_with_index(related_p1), 
                    self.get_position_with_index(related_p2),
        ...
        # for i in range(3 * self.kp_num):
        #     self.grad_f[i] += self.record_force[i // 3][i % 3]
        for i in range(self.kp_num):
    ...
    @ti.kernel
    def step_xv(self, time_step: data_type, enable_ground: bool, sim_mode: bool, theta: data_type) -> int:
        can_step = 0
    ...
    def run(self):
        ...
        while self.window.running:
            ...
            for i in range(3):
                ...
                with ti.ad.FwdMode(loss=self.grad_f, param=self.grad_x, seed=[1.0 if j == 0 else 0.0 for j in range(3 * self.kp_num)]):
                    self.F(self.folding_angle, self.sim_mode, self.gravitational_acc, self.enable_ground, self.tsa_turning_angle)

                print(self.grad_f.dual)
                ...
                flag = self.step_xv(self.dt, self.enable_ground, self.sim_mode, self.folding_angle)
