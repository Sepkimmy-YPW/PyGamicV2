...
# def initTI():
data_type = ti.f64
ti.init(arch=ti.cpu, default_fp=data_type, fast_math=False)
...
class OrigamiSimulator:
    def __init__(self, use_gui=True, debug_mode=False) -> None:
        ...
        self.ori_sim = OrigamiSimulationSystem()
        self.tsa_strings = []
    ...
    def startOnlyTSA(self, units, max_size, total_bias, unit_edge_max):
        ...
        self.vertices = ti.Vector.field(3, dtype=ti.f32, shape=self.kp_num) # 点坐标
        ...
        self.tri_unit_initial_angle = ti.Vector.field(3, dtype=data_type, shape=self.div_indices_num) #每个三角形的初始角度信息
        ...
        self.original_distance_matrix = ti.field(dtype=data_type, shape=(self.kp_num, self.kp_num)) #关键点之间的原始距离矩阵

        self.line_color = ti.Vector.field(3, dtype=ti.f32, shape=self.line_total_indice_num*2) #线段颜色，用于渲染
        ...
        if self.constraint_number == 0:
            ...
            # TSA交叉点
            self.intersection_point = ti.Vector.field(3, dtype=data_type, shape=max(1, sum([self.tsa_root[i][1] - 1 for i in range(len(self.tsa_root))])))
        ...
        self.old_x = ti.Vector.field(3, dtype=data_type, shape=self.kp_num) #点的位置
        self.viscousity = .8e-3
        self.enable_ground = False

        self.next_x = ti.Vector.field(3, dtype=data_type, shape=self.kp_num) #点的位置
        self.next_v = ti.Vector.field(3, dtype=data_type, shape=self.kp_num) #点的速度
        self.answer_x = ti.Vector.field(3, dtype=data_type, shape=self.kp_num)
        self.answer_v = ti.Vector.field(3, dtype=data_type, shape=self.kp_num)
        ...
        self.dF = ti.Matrix.field(3, 3, dtype=data_type, shape=(3, 3))
        self.dE = ti.Matrix.field(3, 3, dtype=data_type, shape=(3, 3)) # E = 0.5 * (F ^ T * F - I)
        self.dP = ti.Matrix.field(3, 3, dtype=data_type, shape=(3, 3)) # P = F(2 * mu * E + landa * tr(E)I)
        self.dH = ti.Matrix.field(3, 3, dtype=data_type, shape=(3, 3)) # H = -AP * dm ^ (-1)

        # self.K = ti.field(data_type, shape=(3 * self.kp_num, 3 * self.kp_num))

        # self.M = ti.field(data_type, shape=(3 * self.kp_num, 3 * self.kp_num))
        # self.AX = ti.field(data_type, shape=(3 * self.kp_num, 3 * self.kp_num))
        ...
        # self.K = ti.Matrix([[0.] * 3 * self.kp_num for i in range(3 * self.kp_num)], dt=data_type)
    ...
    def initialize(self):
        ...
        # 初始化节点质量
        for i in range(self.kp_num):
            self.masses[i] = self.mass_list[i]
        # 初始化三角面索引
        ...
        # for i in range(self.kp_num):
        #     self.M[3 * i, 3 * i] = self.masses[i]
        #     self.M[3 * i + 1, 3 * i + 1] = self.masses[i]
        #     self.M[3 * i + 2, 3 * i + 2] = self.masses[i]
    ...
    def implicit_xv(self, theta: data_type, sim_mode: bool, gravitational_acc: tm.vec3, enable_ground: bool):
        for i in range(self.kp_num):
    ...
    def F(self, theta: data_type, sim_mode: bool, gravitational_acc: tm.vec3, enable_ground: bool, tsa_turning_angle: data_type):
        ...
        for i in range(self.div_indices_num):
            ...
            piola_temp = 2.0 * self.mu * green_tensor + self.landa * ti.Matrix.trace(green_tensor) * new_identity
            piola = f @ piola_temp 
            ...
            for j in ti.static(range(3)):
                for k in ti.static(range(3)):
                    self.dF[j, k] = self.dDs[j, k] @ self.dm[i]
                    self.dE[j, k] = 0.5 * (self.dF[j, k].transpose() @ f + f.transpose() @ self.dF[j, k])
                    self.dP[j, k] = self.dF[j, k] @ piola_temp + f @ (2 * self.mu * self.dE[j, k] + ti.Matrix.trace(self.dE[j, k]) * new_identity)
                    self.dH[j, k] = -self.A[i] * self.dP[j, k] @ self.dm[i].transpose()
                    df1 = tm.vec3([self.dH[j, k][0, 0], self.dH[j, k][1, 0], self.dH[j, k][2, 0]])
                    df2 = tm.vec3([self.dH[j, k][0, 1], self.dH[j, k][1, 1], self.dH[j, k][2, 1]])
                    df0 = -df1 - df2
        ...
        for i in range(self.div_indices_num):
            ...
            for j in range(3):
                for k in range(3):
                    m = 0.
                    if j == k:
                        m = self.masses[indice_list[j] // 3]
                    for jj in range(3):
                        for kk in range(3):
                            builder[indice_list[j] + jj, indice_list[k] + kk] += (m - (h ** 2) * K[3 * j + jj, 3 * k + kk]) if jj == kk else -(h ** 2) * K[3 * j + jj, 3 * k + kk]
    ...
    # @ti.kernel
    def step_xv(self, time_step: data_type, enable_ground: bool, sim_mode: bool, theta: data_type):
        # for i in range(3 * self.kp_num):
        #     for j in range(3 * self.kp_num):
        #         self.AX[i, j] = self.M[i, j] - time_step ** 2 * self.K[i, j]

        self.fill_K(time_step, self.AK)
        self.fill_b(time_step)

        # sparse_solver = ti.linalg.SparseSolver(data_type, "LLT")
        # A = self.AK.build()
        # sparse_solver.analyze_pattern(A)
        # sparse_solver.factorize(A)
        sparse_solver = ti.linalg.SparseSolver(data_type, "LDLT")
        sparse_solver.compute(self.AK.build())
        self.u0.from_numpy(sparse_solver.solve(self.b))

        # print(self.u0)

        for i in range(3 * self.kp_num):
            self.v[i // 3][i % 3] = self.u0[i]
        
        for i in range(self.kp_num):
            self.x[i] = self.x[i] + time_step * self.v[i]
        ...
        # if not can_step:
        #     for i in range(0, self.kp_num):
        #         self.v[i] = self.back_up_v[i] + h * self.dv[i] * min_energy_beta
        #     for i in range(0, self.kp_num):
        #         self.x[i] = self.back_up_x[i] + h * self.v[i]
        #         if enable_ground and self.x[i][Z] < 0.0:
        #             self.x[i][Z] = 0.0
    ...
    @ti.func
    def explicit_step_x(self, time_step: data_type, enable_ground: bool):
        h = time_step
    ...
    @ti.func
    def explicit_step_v(self, time_step: data_type, mode: bool):
        h = time_step
        # -- YOUR CODE BEGINS HERE --
        for i in range(0, self.kp_num):
            if mode:
    ...
    @ti.kernel
    def explicit_step_xv(self, time_step: data_type, enable_ground: bool, mode: bool):
        self.explicit_step_v(time_step, mode)
        self.explicit_step_x(time_step, enable_ground)

    @ti.func
    def verlet_x(self, time_step: data_type, enable_ground: bool):
        h = time_step
        for i in range(self.kp_num):
            temp = self.x[i]
    ...
    @ti.kernel
    def verlet(self, time_step: data_type, enable_ground: bool):
    ...
    @ti.func
    def rk_step_x(self, time_step: data_type, enable_ground: bool):
        h = time_step
        # -- YOUR CODE BEGINS HERE --
        for i in range(0, self.kp_num):
            self.x[i] = self.back_up_x[i] + h * self.v[i]
    ...
    @ti.func
    def rk_step_v(self, time_step: data_type, mode: bool):
        h = time_step
        # -- YOUR CODE BEGINS HERE --
        for i in range(0, self.kp_num):
            if mode:
    ...
    @ti.kernel
    def rk_step_xv(self, time_step: data_type, enable_ground: bool, mode: bool):
    ...
    def RK2(self, time_step: data_type, enable_ground: bool, folding_angle: data_type, sim_mode: bool, gravitational_acc: tm.vec3, mode: bool):
        self.backup_xv()
    ...
    def run(self):
        ...
        while self.window.running:
            if self.window.get_event(ti.ui.PRESS):
                if self.window.event.key == 'shift':
                    pass
